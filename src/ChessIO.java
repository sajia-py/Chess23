...
public class ChessIO {
    ...
    public static String moveString (Pieces pieces, Coordinate coordinate, Piece piece) {
        ...
        // Debugging the current piece and move
        System.out.println("Piece: " + piece.getName() + " from: " + previousCoordinate + " to: " + coordinate);
    ...
    public static String removeAmbiguous(Pieces pieces, Coordinate coordinate, Piece piece) {
        // If the piece is already at the target coordinate
        if (pieces.pieceToSameCoordinate(coordinate, piece)) {

            // If pieces are in the same rank, we can disambiguate using the file
            if (pieces.pieceInSameRank(piece)) {
                return piece.getFile() + ""; // return the file to disambiguate
            }
            // If pieces are in the same file, we can disambiguate using the rank
            else if (pieces.pieceInSameFile(piece)) {
                return piece.getRank() + ""; // return the rank to disambiguate
            }
            // If neither, no ambiguity, return empty
            else {
                return "";
            }
        }
        // Return empty if not the same coordinate
        return "";
    }
