...
public class Pieces {
    ...
    public boolean pieceToSameCoordinate(Coordinate coordinate, Piece piece) {
        // Assert that the piece's potential moves include the given coordinate
        assert piece.getPotentialMoves().contains(coordinate);

        // Handle special case for King
        if (piece.getName() == ID.KING) {
            King king = (King) piece;
            // If the King can move to the coordinate, return true
            if (king.getPotentialMoves().contains(coordinate)) {
                return true;
            }
            return false;  // Return false if the King cannot move to the coordinate
        }

        // Get all pieces of the same color
        HashMap<Coordinate, Piece> coloured = getColourPieces(piece.getColour());

        // Check if any other piece of the same type can move to the given coordinate
        for (Piece value : coloured.values()) {
            // Ensure the piece is not the same as the original piece
            if (value.getName() == piece.getName()
                    && value.getPotentialMoves().contains(coordinate)
                    && !value.equals(piece)) {
                return true;  // Another piece of the same type can move to the coordinate
            }
        }
        return false;  // No other piece of the same type can move to the coordinate
    }
